...
class SARLPWOJLS(nn.Module):
    def __init__(self, backbone, feat_dim, cfg):
        ...
        self.ttd = nn.Sequential(
            nn.Conv1d(text_dim+feat_dim, text_dim+feat_dim, 1),
            nn.BatchNorm1d(text_dim+feat_dim),
            nn.GELU(),
            # nn.ReLU(inplace=True),
            nn.Conv1d(text_dim+feat_dim, feat_dim, 1),
        )
...
def sarlpwoJLS(cfg):
    ...
    model = SARLPWOJLS(backbone, feat_dim, cfg)
