...
#
#
# def visualize_crops_and_original(x, num_images=1):
#     """
#     可视化输入批次中每张图像的 4 个裁剪子图以及原图。
#
#     Args:
#         x (torch.Tensor): 输入张量，形状 [B, 3, 448, 448]
#         num_images (int): 要展示的样本数量
#     """
#     B = x.size(0)
...
#             crop_img = crops[i].permute(1, 2, 0).cpu().numpy()
#             ax.imshow(crop_img)
#             ax.axis('off')
...
class SARLP(nn.Module):
    def __init__(self, backbone, feat_dim, cfg):
        ...
        self.attention2 = LowRankBilinearAttention(512, 512, 256)

        # self.mlp = nn.Sequential(
        #     nn.Conv1d(feat_dim, 512, 1),
        #     nn.BatchNorm1d(512),
        #     # nn.GELU(),
        #     # # # nn.ReLU(inplace=True),
        #     # nn.Conv1d(512, 512, 1),
        # )
        self.fc0 = nn.Conv1d(512, cfg.num_classes, 1)
        ...
        self.fc2 = nn.Sequential(
            ...
            # nn.ReLU(inplace=True),
            nn.Dropout(),
            nn.Linear(512+512, cfg.num_classes),
