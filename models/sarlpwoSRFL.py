...
class WOSRFL(nn.Module):
    def __init__(self, backbone, feat_dim, cfg):
        ...
        self.attention2 = LowRankBilinearAttention(feat_dim, text_dim, 1024)  # comm


        # self.fc0 = nn.Conv1d(feat_dim, cfg.num_classes, 1)
        self.fc2 = nn.Sequential(
            nn.Linear(feat_dim+text_dim, feat_dim+text_dim),
            nn.GELU(),
            # nn.ReLU(inplace=True),
            nn.Dropout(),
            nn.Linear(feat_dim+text_dim, cfg.num_classes),
    ...
    def forward(self, x, y=None, mask_label=None):
        ...
        # x_g = self.gsp(x.transpose(1, 2)).transpose(1, 2)
        v = self.embeddings.unsqueeze(0).repeat(x.shape[0], 1, 1)  # [B, C, dt]  label embedding
        ...
        f = torch.matmul(torch.softmax(alpha2, dim=-1), v)  # [B, P, dt]  # softmax
        f = torch.cat((f, x), dim=-1)  # [B, P, 1024+dt]

        alpha3 = self.fc2(f)  # [B, P, C]
        logits1 = torch.mul(torch.softmax(alpha3, dim=1), alpha3).sum(dim=1)  # fusion
        # logits1 = alpha3.transpose(1, 2).topk(1, dim=-1)[0].mean(dim=-1)  # max
        # logits2 = logits2 @ A
        ...
        if self.training:
            ...
            if self.cfg.partial == 1:
                ...
                # pre0 = (torch.sigmoid(logits0) >= 0.5).float()  # 转成0，1
                # pre1 = (torch.sigmoid(logits1) >= 0.5).float()  # 转成0，1

                mask = mask_label.reshape(logits1.shape[0], -1)  # [B, C]  ，[0, 1],1表示可见，0表示不可见
                # y_new = ((pre0+pre1)*(1-mask)+y.squeeze(1)).detach()
                ...
                # y_sam = y_bar[0].squeeze(1)
                # y_fc = y_bar[1].squeeze(1)
                # y_sam = y_sam.squeeze(1)
                # y_fc = y_fc.squeeze(1)
            ...
            ce_loss = 1 * bce1 + 0 * bce0 + 0 * L_norm + 0 * L_clip
...
def sarlpwoSRFL(cfg):
    ...
    model = WOSRFL(backbone, feat_dim, cfg)
