...
class ModifiedResNet(nn.Module):
    ...
    def forward(self, x, is_fc=False):
        ...
        if not is_fc:
            x = self.final_pool(x)  # [B, 2048, 7, 7]
            x = self.attnpool(x)  # [1+p, B, 512]
...
def build_model_conv_proj(state_dict: dict, cfg):
    ...
    model = CLIP_conv_proj(
        embed_dim,
        cfg.img_size, vision_layers, vision_width, vision_patch_size,
        context_length, vocab_size, transformer_width, transformer_heads, transformer_layers
    )
    ...
    for k, v in old_state_dict.items():
        if k in state_dict.keys():
            if 'visual' in k and ('v_proj' in k or 'c_proj' in k):
                ...
                w_spacial = state_dict[k][1:].permute(1, 0).reshape(1, -1, 7, 7)  # 448 vit-B/32  and RN101
                # w_spacial = state_dict[k][1:].permute(1, 0).reshape(1, -1, 14, 14)  # 448 vit-B/16
