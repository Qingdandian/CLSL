...
def prepare_env(args, argv):
    # prepare data config
    cfg = vars(args)
    org_path = '/public/home/hpc2307070100001/Datasets/all_data/data'
    # org_path = 'E:/做实验时用到的数据集/voc/data'
    cfg['org_path'] = org_path
    cfg['data'] = args.data
    cfg['train_path'] = join(org_path, args.data, 'train.txt')  # default: 'data' E:/做实验时用到的数据集/voc/data
    cfg['test_path'] = join(org_path, args.data, 'test.txt')  # /public/home/hpc2307070100001/Datasets/all_data/data
    cfg['label_path'] = join(org_path, args.data, 'label.txt')
    cfg['embed_path'] = join(org_path, args.data, '{}.npy'.format(cfg['embed_type']))
    cfg['ignore_path'] = join(org_path, args.data, 'ignore.npy')
    cfg['adj_path'] = join(org_path, args.data, 'adj.pkl')
    cfg['label_mask_path'] = join(org_path, args.data, 'partial_label_%.2f.pt' % args.partial)
    # check_exists(cfg['adj_path'])
...
def get_args():
    ...
    parser.add_argument('--model', type=str, default='sarlp', help='default=sarlp，sarlp_clip_lora, baseline')
    parser.add_argument('--clip_model_path', default='D:/pycharm2023/ALL_project/new_practice/GCN_2024/pre_model/clip/RN101.pt', type=str,
                        help='pre model path,default:ViT-B-16,ViT-B-32,RN101')   # /public/home/hpc2307070100001/PreModelEncoder/clip/RN101.pt   D:/pycharm2023/ALL_project/new_practice/GCN_2024/pre_model/clip/RN101.pt
    parser.add_argument('--arch', type=str, default='resnet101')
    parser.add_argument('--data', type=str, default='voc2007')
    parser.add_argument('--seed', type=int, default=0)
    parser.add_argument('--lr', type=float, default=0.0001, help='default=1e-4')
    ...
    # # model:scene
    # parser.add_argument('--k_nums', type=int, default=4)
    # parser.add_argument('--gcn_layers', type=int, default=1)
    # model: partial
    parser.add_argument('--partial', type=float, default=0.08, help=' [0.1 - 1] , when 1, seen all_labels')


    # lora parameters
    parser.add_argument('--use_lora', type=str, default='True')
    parser.add_argument('--lora_r', type=float, default=8, help='LoRa 秩')
    parser.add_argument('--lora_alpha', type=float, default=32, help='缩放因子')
    parser.add_argument('--lora_dropout', type=float, default=0.01, help='Dropout率')
    ...
    parser.add_argument('--lamda', type=float, default=1, help='deault=1.0')
    parser.add_argument('--lamda2', type=float, default=100, help='deault=1.0')
    parser.add_argument('--tau', type=float, default=1, help='deault=1.0')
    ...
    parser.add_argument('--ema-decay', type=float, default=0.9997, help='deault=0.9997')
    parser.add_argument('--pct-start', type=float, default=0.2)
    
    parser.add_argument('--batch-size', type=int, default=1, help='default=64')
    parser.add_argument('--img-size', type=int, default=448, help='default=448')
