...
class AsymmetricLossOptimized(nn.Module):
    ...
    def forward(self, x, y, sigmoid=True, MASK=False):
        """"
        Parameters
        ----------
        x: input logits
        ...
        #     self.loss *= self.mask
        #     with torch.no_grad():
        #         row_sums = (self.mask.sum(dim=1) != 0).sum()
        #         zero_mask = (self.mask.sum(dim=1) == 0)
        #         if zero_mask.any():
        #             self.mask[zero_mask, 0] = 1  # 将全-1行的第一个位置设为1
        #         if row_sums == 0:
        #             row_sums = 1
        #     _loss = - self.loss / row_sums
        #     _loss = (_loss.sum(dim=1) / self.mask.sum(dim=1)).sum() * 1000
        # else:
        #     _loss = - self.loss.sum() / x.size(0)
        #     _loss = _loss / y.size(1) * 1000
