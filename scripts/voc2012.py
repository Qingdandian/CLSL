...
data_dir = '/public/home/hpc2307070100001/Datasets/all_data/datasets/VOC2012'
anno_dir_trainval = os.path.join(data_dir, 'Annotations_trainval')
image_dir_trainval = os.path.join(data_dir, 'JPEGImages_trainval')
anno_dir_test = os.path.join(data_dir, 'Annotations_test')
image_dir_test = os.path.join(data_dir, 'JPEGImages_test')
# save_dir = 'data/voc2012'
save_dir = '/public/home/hpc2307070100001/Datasets/all_data/data/voc2012'
...
train_txt = os.path.join(data_dir, 'ImageSets_trainval/Main/trainval.txt')
train_imgIds = [t.strip() for t in open(train_txt)]
train_data = []
for img_id in train_imgIds:
    image_path = os.path.join(image_dir_trainval, '{}.jpg'.format(img_id))
    if not os.path.exists(image_path):
        raise Exception('file {} not found!'.format(image_path))
    xml_path = os.path.join(anno_dir_trainval, '{}.xml'.format(img_id))
...
test_txt = os.path.join(data_dir, 'ImageSets_test/Main/test.txt')
test_imgIds = [t.strip() for t in open(test_txt)]
test_data, ignore = [], []
for img_id in test_imgIds:
    image_path = os.path.join(image_dir_test, '{}.jpg'.format(img_id))
    if not os.path.exists(image_path):
        raise Exception('file {} not found!'.format(image_path))
    xml_path = os.path.join(anno_dir_test, '{}.xml'.format(img_id))
    data = parse_xml(xml_path)
    labels = [label for label, diffts in data.items() if sum(diffts) != len(diffts)]
